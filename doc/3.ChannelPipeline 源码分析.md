...
![ChannelHandlerContext之间关系](3.ChannelPipeline 源码分析.assets/ChannelHandlerContext之间关系.png)
...
# .handler和.childHandler 的区别？

.handler 加入的channelhandler 直接加入到 pipeline中， .childHandler 加入的 channelHandler 加入到ServerBootstrapAcceptor中使用
...
如果配置了自动读取，则在执行channelActive事件的时候，会触发一次 read 操作，交给的是 TailContext执行，按照从 tail 节点往前的 outBound 类型 handler 传递
